...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in eox.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("eox.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* EOX features */
    { "eox",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "eox",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "eox",               "gobject",                &gobject,                true,  {} },
    { "eox",               "voteraw",                &voteraw,                true,  {} },
