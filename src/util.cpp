...
const char * const BITCOIN_CONF_FILENAME = "eox.conf";
const char * const BITCOIN_PID_FILENAME = "eoxd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "eox" is a composite category enabling all EOX-related debug output
                if(ptrCategory->count(std::string("eox"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EOX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EOX
    // Mac: ~/Library/Application Support/EOX
    // Unix: ~/.eox
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EOX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EOX";
#else
    // Unix
    return pathRet / ".eox";
#endif
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty eox.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override eox.conf
